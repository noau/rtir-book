...
Then we will also need a type representing the virtual camera. The ray starts at the center of our camera, and its direction is determined by sample point of the pixel on the film plane. So what we only need for virtual camera type is those two things. However, to allow more different types of camera, the process for generating a sample ray for a pixel will be extracted to some types. Here's why.

The most intuitive way to generate a sample way is described as above, the ray crossing the center and the sample point. However, there're multiple different way to choose the two points. When you generate in the way above, you'll get an effect of so called perspective projection. But there's also orthogonal projection. There's also a trick to implement the caustics, that is to randomly choose a start point in a small area around the center. Another trick about this is how to choose the sample point on the film for a pixel. We can choose a random point around within the pixel, but we can also divide the pixel by the number of sample count, and choose one pixel for one small area, called jittering, or stratified samples. The theory will be introduced later, but it usually provides a better result. So we can use two types to model this - `SampleStartGenerator` and `SamplePointGenerator`.

::: callout-note
## The two auxiliary types

The logic for generating sample point and sample ray origin is actually included in the camera instead of in the types, so they're actually just classifying types to determine the behavior of the camera, however you can still move the logic in them, which may give you more flexibility. See [Exercise @sec-p1c4-qcfp-trait-generator].
:::
...
Film will not be included in the camera. Actually, this camera is just a virtual camera as a rendering helper.
...
This is actually easy and intuitive. Consider even to extract new `trait` for each one of them and make it more flexible, although unnecessary and useless most of the time.